...
//hard coded variables for counter-balancing 
var options = {
...
//counting arrays [0:numTrials-1] for iterating through trial specific values in the test_block loop
var stimCounter = [];
...
//increases ITIs to appropriate lengths
var get_ITI = function(iti) {
  return 4500 + iti*1000 // 500 minimum ITI
 }
...
//functions to iterating the counting arrays during the test_block loop
...
for (i = 0; i < ITIs[1].length; i++) {
  ...
  stimulus: function() { //use scan_order and stimPop (an iterator) to get correct stimulus from stim_index array
    ...
    return ('<div class = dd-stim><div class = amtbox style = "color:white">$'+options.large_amt[index]+'</div><br><br>'+
  ...
  data: function() { //use scan_order and dataPop (an iterator) to get correct values for small_amount, large_amount, and later_delay from the options object
    var scan_order = jsPsych.data.getDataByTrialIndex(0).scanner_order;
    var index = stim_index[scan_order][dataPop()];
    return {
...
discount_fixed__fmri_experiment.push(end_block);