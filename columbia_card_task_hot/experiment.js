...
var getBoard = function(board_type) {
	...
	for (var i = 1; i < 33; i++) {
		board += "<div class = square><input type='image' id = c" + i +
...
var getReward = function() {
	...
	for (k = 0; k < tempData.length; k++) {
		if (jsPsych.data.getDataByTrialIndex(k).whichButtonWasClicked == 'endRoundButton' && jsPsych.data
			.getDataByTrialIndex(k).exp_stage == 'test' || jsPsych.data.getDataByTrialIndex(k).whichButtonWasClicked ==
			'collectButton' && jsPsych.data.getDataByTrialIndex(k).exp_stage == 'test') {
...
var practiceSetup =
	"<div class = center-text id = instruct1><div style = 'font-size: 90%'; class = center-block-text><strong>Practice 1: </strong> As you click on cards, you can see your Round Total change in the box in the upper right.  If you turn over a few cards and then want to stop and go to the next round, click the <strong>Stop/Turn Over</strong> button and then <strong>Next Round</strong>.  If turning over cards seems too risky, you can click the <strong>No Card</strong> button, in which case your score for the round will automatically be zero.  This is a practice round, that looks just like the game you will play.  Please select the number of cards you would turn over, given the number of loss cards and the amounts of the gain and loss cards shown below.</div></div> " +
	"<div class = titlebox><div class = center-text><strong>How many cards do you want to take?</strong></div></div>" +
	"<div class = titleboxRight><div id = current_round class = center-text>Current Round:  </div></div>" +
...
var instructions_block = {
	...
	pages: [
		...
		'<div class = centerbox-CCT><p class = block-text><div class = center-text id = instruct1><strong>Example 1: </strong>In the example below, you see 32 unknown cards. The display shows you that 1 of these cards is a loss card. It also tells you that turning over each gain card is worth 10 points to you, and that turning over the loss card will cost you 750 points. Let us suppose you decided to turn over 7 cards and then decided to stop. Please click the "See Result" button to see what happens: </div>' +
		...
		'<div class = centerbox-CCT><p class = block-text><div class = center-text id = instruct2><strong>Example 2: </strong>In the example below, you see 32 unknown cards. The display shows you that 3 of these cards is a loss card. It also tells you that turning over each gain card is worth 30 points to you, and that turning over the loss card will cost you 250 points. Let us suppose you decided to turn over 10 cards and then decided to stop. Please click the "See Result" button to see what happens: </div>' +
		"<div class = instructBox><div><strong>How many cards do you want to take?</strong></div></div>" +
