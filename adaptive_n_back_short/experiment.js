...
var num_blocks = 15 // number of adaptive blocks
...
var start_control_block = {
	type: 'poldrack-text',
	text: '<div class = centerbox><p class = block-text>In this block you do not have to match letters to previous letters. Instead, press the left arrow key every time you see a "t" or "T" and the down arrow key for all other letters.</p><p class = center-block-text>Press <strong>enter</strong> to begin.</p></div>',
	cont_key: [13],
	data: {
		trial_id: "instruction"
	},
...
//Set up experiment
var adaptive_n_back_short_experiment = []
adaptive_n_back_short_experiment.push(instruction_node);
adaptive_n_back_short_experiment.push(start_practice_block)
adaptive_n_back_short_experiment = adaptive_n_back_short_experiment.concat(practice_trials)

if (control_before === 0) {
	adaptive_n_back_short_experiment.push(start_control_block)
	adaptive_n_back_short_experiment = adaptive_n_back_short_experiment.concat(control_trials)
}
for (var b = 0; b < num_blocks; b++) {
	adaptive_n_back_short_experiment.push(start_adaptive_block)
	adaptive_n_back_short_experiment.push(adaptive_test_node)
	if ($.inArray(b, [4, 7, 15]) != -1) {
		adaptive_n_back_short_experiment.push(attention_node)
	}
	adaptive_n_back_short_experiment.push(update_delay_block)
}
...
//Set up control
adaptive_n_back_short_experiment.push(post_task_block)
adaptive_n_back_short_experiment.push(end_block)
