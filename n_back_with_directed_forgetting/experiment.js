...
var createTrialTypes = function (numTrialsPerBlock, delay) {
  ...
  for (
    ...
    numIterations <
    numTrialsPerBlock /
      (n_back_conditions.length * directed_forgetting_conditions.length);
    numIterations++
  ) {
    for (
      var numNBackConds = 0;
      ...
      for (
        var numShapeConds = 0;
        numShapeConds < directed_forgetting_conditions.length;
        numShapeConds++
      ) {
        directed_forgetting_condition =
          directed_forgetting_conditions[numShapeConds];
  ...
  for (i = 0; i < stim_len; i++) {
    if (i < 3) {
      ...
      if (n_back_condition == "match") {
        probe = randomDraw([
          remember_letters[remember_letters.length - delay].toLowerCase(),
          remember_letters[remember_letters.length - delay].toUpperCase(),
        ]);
        correct_response = possible_responses[0][1];
      } else if (n_back_condition == "mismatch") {
        probe = randomDraw(
...
var preFileType =
  "<img class = center src='/static/experiments/n_back_with_directed_forgetting/images/";

var n_back_conditions = jsPsych.randomization.repeat(
  ["match", "mismatch", "mismatch", "mismatch", "mismatch"],
  1
);
var directed_forgetting_conditions = jsPsych.randomization.repeat(
  ["forget", "remember"],
  1
);
...
var prompt_text_list =
  ...
  "<li>Match the current letter to the letter that appeared " +
  delay +
  " trial(s) ago.</li>" +
  "<li>Press the " +
  possible_responses[0][0] +
  " if the letters match.</li>" +
  "<li>Press the " +
...
var prompt_text =
  ...
  '<p class = center-block-text style = "font-size:16px; line-height:80%%;">Match the current letter to the letter that appeared ' +
  delay +
  " trial(s) ago.</p>" +
  '<p class = center-block-text style = "font-size:16px; line-height:80%%;">Press the ' +
  possible_responses[0][0] +
  " if the letters match.</p>" +
  '<p class = center-block-text style = "font-size:16px; line-height:80%%;">Press the ' +
  possible_responses[1][0] +
  ". if they mismatch.</p>" +
  "</div>";
...
var task_boards = [
  [
    '<div class = bigbox><div class = centerbox><div class = cue-text><font size="10">',
  ],
  ["</font></div></div></div>"],
];
var task_boards = [
...
var instructions_block = {
  ...
  pages: [
    ...
    "<div class = centerbox>" +
      ...
      '<p class = block-text style="font-size:20px">Press the ' +
      possible_responses[0][0] +
      " if the letters match, and the " +
      possible_responses[1][0] +
      " if they mismatch.</p>" +
...
var start_test_block = {
  ...
  on_finish: function () {
    feedback_text =
      ...
      delay +
      " trial(s) ago <i>in your memory set</i>. Press enter to begin.";
  },
};
