...
function addID() {
  jsPsych.data.addDataToLastTrial({
    exp_id: "stop_signal_with_directed_forgetting_prolific",
  });
}
...
var createTrialTypes = function (numTrialsPerBlock) {
  ...
  for (
    ...
    numIterations <
    numTrialsPerBlock /
      (directed_cond_array.length * stop_signal_conditions.length);
    numIterations++
  ) {
    for (
  ...
  for (var i = 0; i < new_len; i++) {
    ...
    var correct_response = getCorrectResponse(
      directed_condition,
...
var preFileType =
  '<img class = center src="/static/experiments/stop_signal_with_directed_forgetting_prolific/images/';

var task_boards = [
  ["<div class = bigbox><div class = topLeft><div class = cue-text>"],
  ["</div></div><div class = topMiddle><div class = cue-text>"],
  ["</div></div><div class = topRight><div class = cue-text>"],
  ["</div></div><div class = bottomLeft><div class = cue-text>"],
  ["</div></div><div class = bottomMiddle><div class = cue-text>"],
  ["</div></div><div class = bottomRight><div class = cue-text>"],
  ["</div></div></div>"],
];
...
var pathSource =
  "/static/experiments/stop_signal_with_directed_forgetting_prolific/images/";
var images = [];
...
var feedback_text =
  "Welcome to the experiment. This experiment will take about 50 minutes. Press <i>enter</i> to begin.";
var feedback_block = {
...
var instructions_block = {
  ...
  pages: [
    "<div class = centerbox>" +
      ...
      numLetters +
      " letters on each trial. There will be " +
      numLetters / 2 +
      " at the top and " +
      numLetters / 2 +
      " on the bottom. You must memorize all " +
      numLetters +
      " letters.</p> " +
      "<p class = block-text>There will be a short delay, then you will see a cue, either <i>TOP</i> or <i>BOT</i>. " +
      "This will instruct you to <i>FORGET</i> the " +
      numLetters / 2 +
      " letters located at either the top or bottom (respectively) of the screen.</p>" +
      "<p class = block-text>The " +
      numLetters / 2 +
      " remaining letters that you must remember are called your <i>MEMORY SET</i>. Please forget the letters not in the memory set.</p>" +
      "<p class = block-text>So for example, if you get the cue TOP, please <i>forget the top " +
      numLetters / 2 +
      " letters</i> and remember the bottom " +
      numLetters / 2 +
      " letters. <i>The bottom " +
      numLetters / 2 +
      " letters would be your MEMORY SET.</i></p>" +
      "</div>",
...
var start_test_block = {
  ...
  text:
    ...
    "<p class = block-text>In this experiment you will be presented with " +
    numLetters +
    " letters on each trial. You must memorize all " +
    numLetters +
    " letters.</p> " +
    "<p class = block-text>There will be a short delay, then you will see a cue, either <i>TOP</i> or <i>BOT</i>. " +
    "This will instruct you to <i>FORGET</i> the " +
    numLetters / 2 +
    " letters located at either the top or bottom (respectively) of the screen. " +
    "The " +
    numLetters / 2 +
    " remaining letters that you must remember are called your <i>MEMORY SET</i>. Please forget the letters not in the memory set.</p>" +
    "<p class = block-text>So for example, if you get the cue TOP, please forget the top " +
    numLetters / 2 +
    " letters and remember the bottom " +
    numLetters / 2 +
    " letters.</p>" +
...
var practiceNode = {
  ...
  loop_function: function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (
        data[i].trial_id == "practice_trial" &&
        data[i].stop_signal_condition == "go"
      ) {
        total_trials += 1;
        go_trials += 1;
        if (data[i].rt != -1) {
        ...
        data[i].trial_id == "practice_trial" &&
        data[i].stop_signal_condition == "stop"
      ) {
        total_trials += 1;
        stop_trials += 1;
        if (data[i].rt != -1) {
    ...
    if (
      ...
      stop_acc < maxStopCorrectPractice &&
      stop_acc > minStopCorrectPractice
    ) {
      feedback_text +=
...
stop_signal_with_directed_forgetting_prolific_experiment = [];

stop_signal_with_directed_forgetting_prolific_experiment.push(practiceNode);
stop_signal_with_directed_forgetting_prolific_experiment.push(feedback_block);

stop_signal_with_directed_forgetting_prolific_experiment.push(start_test_block);
stop_signal_with_directed_forgetting_prolific_experiment.push(testNode);
stop_signal_with_directed_forgetting_prolific_experiment.push(feedback_block);

stop_signal_with_directed_forgetting_prolific_experiment.push(post_task_block);
stop_signal_with_directed_forgetting_prolific_experiment.push(end_block);
