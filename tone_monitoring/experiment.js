...
var instructions_block = {
  type: 'instructions',
  pages: [
    '<div class = centerbox><p class = block-text>In this experiment you will hear tones in a sequence, one after the other. You will hear one of three tones on each trial: a high tone, a medium tone, or a low tone.</p><p class = block-text>Use the <strong>right arrow key</strong> to advance through the instructions. You can go back using the <strong>left arrow key</strong>.</p></div>',
    '<div class = centerbox><p class = block-text>Your job is to keep each track of the number of times each tone repeats and respond when you hear any tone repeat four times by pressing the spacebar. For instance, if you hear "high, high, low, medium, high, low, medium, <strong>high</strong>" you should respond on the last (fourth) high tone.</p><p class = block-text>If the sequence of tones continued with "high, medium, low, <strong>medium</strong>" you would respond again, as the medium tone had repeated four times, and so on.</p><p class = block-text>Use the <strong>right arrow key</strong> to advance through the instructions. You can go back using the <strong>left arrow key</strong>.</p></div>',
	"<div class = centerbox><p class = block-text>After you respond by pressing the spacebar, you should 'reset' that tone's count. So in the previous examples, once you press the spacebar in response to the high tones, you should start counting the high tones again from 0.</p><p class = block-text>Even if you believe you pressed the spacebar at the wrong time (if you thought only 3 high tones had passed instead of 4), you <strong>still should reset your count</strong>. So if you count 3 high tones and inappropriately responded, begin counting high tones from 0 again.</p><p class = block-text>Use the <strong>right arrow key</strong> to advance through the instructions. You can go back using the <strong>left arrow key</strong>.</p></div>",
	'<div class = centerbox><p class = block-text>To summarize, you will keep track of three different tones: low, medium and high. When you count 4 of any tone, press the spacebar. After you respond to a tone (regardless of if you were correct or not), mentally reset that tones count, while leaving the count for the other tones intact.</p><p class = block-text>Use the <strong>right arrow key</strong> to hear the three tones (you will not be able to go back afterwards). You can go back using the <strong>left arrow key</strong>.</p></div>'
	]
};
