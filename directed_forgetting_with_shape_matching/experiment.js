...
var createTrialTypes = function (numTrialsPerBlock) {
  ...
  for (
    ...
    numIterations <
    numTrialsPerBlock /
      (directed_cond_array.length * shape_matching_conditions.length);
    numIterations++
  ) {
    for (
  ...
  for (var i = 0; i < previous_len; i++) {
    ...
    if (shape_matching_condition == "match") {
      ...
      distractor = randomDraw(
        stimArray.filter(function (y) {
          return $.inArray(y, [probe]) == -1;
        })
      );
    }
...
var preFileType =
  '<img class = center src="/static/experiments/directed_forgetting_with_shape_matching/images/';
...
var pathSource =
  "/static/experiments/directed_forgetting_with_shape_matching/images/";
var images = [];
...
for (i = 0; i < stimArray.length; i++) {
  images.push(pathSource + "red_" + stimArray[i] + ".png");
}
...
var start_test_block = {
  ...
  text:
    "<div class = centerbox>" +
    "<p class = block-text>This test will be exactly like practice. You will be presented with " +
    numLetters +
