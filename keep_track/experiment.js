...
function addID() {
  jsPsych.data.addDataToLastTrial({'exp_id': 'keep_track'})
}
...
var feedback_instruct_block = {
  type: 'poldrack-text',
  cont_key: [13],
  data = {exp_id: 'keep_track', trial_id: 'instruction'},
...
var end_block = {
  type: 'poldrack-text',
  timing_response: 60000,
  data = {exp_id: 'keep_track', trial_id: 'end'},
...
var start_practice_block = {
  type: 'poldrack-text',
  timing_response: 60000,
  data = {exp_id: 'keep_track', trial_id: 'practice_intro'},
...
var end_practice_block = {
  type: 'poldrack-text',
  timing_response: 60000,
  data = {exp_id: 'keep_track', trial_id: 'practice_end'},
...
var start_test_block = {
  type: 'poldrack-text',
  timing_response: 60000,
  data = {exp_id: 'keep_track', trial_id: 'test_intro'},
...
data = {exp_id: 'keep_track', trial_id: 'practice_prompt', exp_stage: "practice", condition: 'target_length_' + target.length, targets: target}
var prompt_block = {
...
var wait_block = {
	...
	stimulus: ' ',
	is_html: true,
	data: {exp_id: 'keep_track', trial_id: 'wait', exp_stage: 'practice'},
	choices: 'none',
	prompt: prompt,
	timing_stim: 500,
	timing_response: 500,
...
for (i = 0; i < block.length; i++ ) {
	...
	data = {exp_id: 'keep_track', trial_id: 'practice_' + block[i][0], exp_stage: "practice", condition: 'target_length_' + target.length, targets: target}
	var track_block = {
		type: 'poldrack-single-stim',
		stimulus: stim,
		is_html: true,
...
for (b=0; b<blocks.length; b++) {
	...
	target = targets[b]
	prompt = '<div class = promptbox><div class = prompt-text>Targets: ' + target.join(', ') +'</div></div>'
	data = {exp_id: 'keep_track', trial_id: 'test_prompt', exp_stage: "test", condition: 'target_length_' + target.length, targets: target}
	var prompt_block = {
		type: 'poldrack-single-stim',
		stimulus: '<div class = centerbox><p class = block-text>Below are the target categories. They will remain on the bottom of the screen during the trial. Press enter when you are sure you can remember them. </p></div>',
		is_html: true,
	...
	for (i = 0; i < block.length; i++ ) {
		stim = '<div class = centerbox><div class = keep-track-text>' + block[i][1] + '</div></div>'
		data = {exp_id: 'keep_track', trial_id: 'test_' + block[i][0], exp_stage: "test", condition: 'target_length_' + target.length, targets: target}
		var track_block = {
			type: 'poldrack-single-stim',
			stimulus: stim,
			is_html: true,
